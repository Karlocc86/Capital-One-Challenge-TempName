...
- **Plan de rescate estructurado por áreas** (`schemas.FinancialRescuePlan`): `summary`, `insolvency_warning` y `recommendations[]` — **exactamente 6, una por área**: `fin_de_mes` (¿llega? ¿cuánto recortar?), `suscripciones`, `comida_chatarra` (dinero **y salud**), `gastos_hormiga`, `ahorro` (apartar el sobrante), `integral` (el cambio de fondo). Cada una: `title` (≤ 8 palabras), `description` (1-2 frases con montos y nombres reales), `estimated_impact`, `priority` (1-6). Los números vienen del backend: `forecast.month_end` (`MonthEndAnalysis`: ingreso vs egreso mensual, déficit, saldo proyectado al 30, ¿llega?) y `categories.spending_breakdown` (totales de chatarra / hormiga / suscripciones); Gemini solo redacta. El fallback sin LLM produce las mismas 6 áreas con los números reales. `web/components/ForecastCard.tsx` ya pinta esta lista (aún sin montar en el dashboard).
